...
func PublishBlog(c echo.Context) error {
	...
	url, err := StorageDBFile(c, &resp, "/blog/")
	if err != nil {
		return err
	}
	...
	//增加积分
	detail := PointDetails{userID, "publish blog", 10}
...
func UploadContent(c echo.Context) error {
	...
	defer utils.ResponseData(c, &resp)
	//2. 保存文件
	url, err := StorageDBFile(c, &resp, "/content/")
	if err != nil {
		return err
	}
...
//上传文件：可以是md或photo
func StorageDBFile(c echo.Context, resp *utils.Resp, dir string) (url string, err error) {
	//1. 解析数据
...
func ListsAll(c echo.Context) error {
	...
	var blogs []Blog
	err := db.Dbconn.QueryAll("blogplatform", "blogs", bson.M{"userid": userID}, &blogs, 0, 0)
	if err != nil {
		fmt.Println("failed to query all", err)
		return err
	}
...
func GetDetails(c echo.Context) error {
	...
	err := db.Dbconn.QueryAll("blogplatform", "details", bson.M{"userid": userID}, &details, 0, 0)
	if err != nil {
		fmt.Println("failed to query all", err)
		return err
	}
